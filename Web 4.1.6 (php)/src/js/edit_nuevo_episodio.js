...
$( ".new_guardar" ).click(function() {
    ...
    listo_para_agregar = true
    bien_mal_form('new_tipo', 'Podcast')
    bien_mal_form('new_titulo', '')
    bien_mal_form('new_episodio', '')
    bien_mal_form('new_audio', 'Audio')

    if(listo_para_agregar){
        var json = crearJson()
    }
})
...
function guardarRss(json){
    //console.log(json)
    var rss_audio = rss_anchor[$('.new_audio').val().split(' - ')[0]]
    $.ajax({
        url: "../../src/php/agregar_episodio_en_rss.php",
        dataType: 'text',
        cache: false,
        data: { json : json, rss : rss_audio },
        type: 'post',
        success: function(data){
            //console.log('rss')
            console.log(data)
